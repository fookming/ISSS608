...
format: 
  html:
    ...
    number-offset: [14, 0]  # Offset heading levels by starting at 15
output: 
  html_document: 
    toc: true
    tabset: true
...
Rotating x-axis text labels to 30 degrees makes the label overlap with
the plot and we can avoid this by adjusting the text location using `hjust` argument to theme’s text element with `element_text()`. We use `axis.text.x` as we want to change the look of x-axis text.
